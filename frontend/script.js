...
const fetchCats = async () => {
  const response = await fetch("http://192.168.0.121:8000/cats");
...
const uneat = async (ate_id) => {
  const response = await fetch(`http://192.168.0.121:8000/uneat/${ate_id}`, {
    method: "DELETE",
  });
  if (response.ok) {
...
const ateToday = async (cat) => {
  const response = await fetch(
    `http://192.168.0.121:8000/ate/today/${cat.name}`
  );
...
const feedCat = async (cat) => {
  const response = await fetch("http://192.168.0.121:8000/feed", {
    method: "POST",
    headers: {
